...
void ShortestWay(unordered_map<int, Pipe> pipeline, int IDout, int IDin, vector <int> tops)
{
	...
	vector<vector<int>> a; //матрица св€зей
	a.resize(SIZE);
	vector<int> d;         //миниммальное рассто€ние
	d.resize(SIZE);
	...
	do {
		...
		{
			if ((v[i] == 1) && (d[i] < min))    // ≈сли вершину ещЄ не обошли и вес меньше min, то переприсваиваем значени€
			{
				min = d[i];
				minindex = i;
			}
		}
		// ƒобавл€ем найденный минимальный вес к текущему весу вершины и сравниваем с текущим минимальным весом вершины
		if (minindex != 10000)
		{
			for (int i = 0; i < SIZE; i++)
			{
	...
	while (end != begin_index)                  // пока не дошли до начальной вершины
	{
		for (int i = 0; i < SIZE; i++)          // просматриваем все вершины
			if (a[i][end] != 0)                 // если св€зь есть
			{
				int temp = weight - a[i][end];  // определ€ем вес пути из предыдущей вершины
